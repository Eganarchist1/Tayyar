...
$infraEnvPath = Join-Path $repoRoot "infra\.env.free-first"
...
$AppDomain = Ensure-Value $AppDomain "APP_DOMAIN"
$ApiDomain = Ensure-Value $ApiDomain "API_DOMAIN"
$AcmeEmail = Ensure-Value $AcmeEmail "ACME_EMAIL"
...
$publicApiUrl = "https://${ApiDomain}"
$publicAppUrl = "https://${AppDomain}"

$otpDeliveryMode = if ($EnableWhatsApp) { "WHATSAPP" } else { "CONSOLE" }
...
Write-Host ""
Write-Host "Next:"
Write-Host "1. Fill SMTP and WhatsApp values only if you want them live now."
Write-Host "2. Copy infra/.env.free-first to the VM as .env."
Write-Host "3. Run docker compose -f infra/docker-compose.free-first.yml up -d"
