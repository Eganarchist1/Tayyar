...
import { ArrowUpRight, Building2, MapPin, Save, Trash2 } from "lucide-react";
...
import { auditSummaryLine, duplicateConfidenceLabel, duplicateReasonLabel, localizeAuditAction } from "@/lib/ops";
...
export default function AdminCustomerDetailPage() {
  ...
  const loadCustomer = React.useCallback(async () => {
    if (!params?.id) return;
    const data = await apiFetch<CustomerDetail>(`/v1/admin/customers/${params.id}`, undefined, "ADMIN");
    setCustomer(data);
    setForm({
      name: data.name || "",
      phone: data.phone,
  ...
  return (
    <PageShell
      ...
      pageTitle={{ ar: "ملف العميل", en: "Customer record" }}
      pageSubtitle={{ ar: "شوف العلاقة بين العميل والتاجر والعناوين في شاشة واحدة.", en: "View customer, merchant, and address context in one place." }}
      showLive
    >
      <div className="space-y-8">
        <PageHeader
          eyebrow={{ ar: "سجل العميل", en: "Customer record" }}
          title={{ ar: customer?.name || "عميل من غير اسم", en: customer?.name || "Unnamed customer" }}
          subtitle={{ ar: "عدّل البيانات الأساسية والعناوين، وراجع آخر الطلبات بسرعة.", en: "Edit core customer data and review recent activity quickly." }}
          breadcrumbs={[
            { label: { ar: "لوحة التحكم", en: "Dashboard" }, href: "/admin" },
            { label: { ar: "العملا", en: "Customers" }, href: "/admin/customers" },
            { label: { ar: "ملف العميل", en: "Customer record" } },
          ]}
          chips={customer ? [
            { label: { ar: customer.merchant.nameAr || customer.merchant.name, en: customer.merchant.name }, tone: "gold" },
            { label: { ar: `${customer.totalOrders} طلب`, en: `${customer.totalOrders} orders` }, tone: "primary" },
          ] : undefined}
        />
        ...
        {!loading && !customer ? (
          <EmptyStateCard
            title={{ ar: "السجل مش موجود", en: "Customer record not found" }}
          ...
          <form className="grid gap-6 xl:grid-cols-[0.84fr_1.16fr]" onSubmit={handleSave}>
            <Card variant="elevated" className="space-y-5">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-black">{tx(locale, "تحكم كامل في بيانات العميل", "Full control over customer data")}</h2>
              </div>
              ...
              <div className="rounded-[24px] border border-white/8 bg-white/[0.03] p-4 text-sm text-text-secondary">
                <div className="mb-2 flex items-center gap-2 text-text-primary">
                  ...
                  {tx(locale, "التاجر المرتبط", "Linked merchant")}
                </div>
                <div>{customer.merchant.nameAr || customer.merchant.name}</div>
              </div>
            ...
            <div className="space-y-6">
              <Card className="space-y-5">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-black">{tx(locale, "العناوين اللي المنصة شغالة بيها", "Addresses currently used by the platform")}</h2>
                </div>
              ...
              <Card className="space-y-5">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <h2 className="mt-2 text-2xl font-black">{tx(locale, "آخر تعاملات العميل على المنصة", "Latest platform activity for this customer")}</h2>
                  </div>
                  <Link href={`/admin/merchants/${customer.merchant.id}`} className="text-sm font-bold text-primary-300">
                ...
                <div className="space-y-3">
                  {customer.recentOrders.map((order) => (
                    <Link key={order.id} href="/admin/orders" className="block rounded-[22px] border border-white/8 bg-white/[0.03] p-4 transition-colors hover:border-primary-500/20 hover:bg-primary-500/8">
              ...
              <Card className="space-y-5">
                <div>
                  <p className="subtle-label">{tx(locale, "مراجعة التشابه", "Duplicate review")}</p>
                  <h2 className="mt-2 text-2xl font-black">{tx(locale, "حالات قريبة من نفس العميل", "Possible duplicate matches")}</h2>
                </div>
                {customer.duplicateCandidates?.length ? (
                  <div className="space-y-3">
                    {customer.duplicateCandidates.map((candidate) => (
                      <div key={candidate.id} className="rounded-[22px] border border-white/8 bg-white/[0.03] p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <div className="font-bold text-text-primary">{candidate.name || tx(locale, "عميل من غير اسم", "Unnamed customer")}</div>
                            <div className="mt-1 text-sm text-text-secondary">{candidate.phone}</div>
                            <div className="mt-1 text-sm text-text-secondary">{candidate.merchant.nameAr || candidate.merchant.name}</div>
                          </div>
                          <div className="rounded-full border border-accent-500/20 bg-accent-500/10 px-3 py-1 text-xs font-bold text-amber-200">
                            {duplicateConfidenceLabel(candidate, locale)}
                          </div>
                        </div>
                        <div className="mt-3 flex flex-wrap gap-2">
                          {candidate.reasons.map((reason) => (
                            <span key={reason} className="rounded-full border border-white/10 bg-white/[0.05] px-3 py-1 text-xs text-text-secondary">
                              {duplicateReasonLabel(reason, locale)}
                            </span>
                          ))}
                        </div>
                      </div>
                  ...
                  <div className="rounded-[22px] border border-dashed border-white/10 bg-white/[0.02] p-4 text-sm text-text-secondary">
                    {tx(locale, "مفيش حالات شبه واضحة حالياً.", "No likely duplicate matches right now.")}
                  </div>
                )}
              </Card>
              ...
              <Card className="space-y-5">
                ...
                {customer.auditTrail?.length ? (
                  <div className="space-y-3">
                    {customer.auditTrail.map((event) => (
